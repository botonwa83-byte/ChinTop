    ...
    var body: some View {
        ZStack {
            ...
            ScrollView(showsIndicators: false) {
                VStack(spacing: 18) {
                    Image(systemName: "book.closed.fill").font(.system(size: 46, weight: .bold)).foregroundStyle(.pink).padding(24).background(.pink.opacity(0.18), in: Circle()).padding(.top, 42)
                    Text("CHIN TOP").font(.system(size: 30, weight: .heavy, design: .rounded)).tracking(3).foregroundStyle(.white)
                    Text("语 文 登 顶 · 得 分 导 航").font(.subheadline).tracking(2).foregroundStyle(.white.opacity(0.65))
                    Text("赋予学生一项能力：证据表达力").font(.headline).multilineTextAlignment(.center).foregroundStyle(.pink).padding(.top, 12)
                    ...
                    HStack { stat("\(ChinModule.all.count)", "语文专题"); Divider().frame(height: 28); stat("\(ChinDailyTaskCatalog.all.count)", "能力任务"); Divider().frame(height: 28); stat("\(ChinCapability.all.count)", "能力方向") }.padding().background(.white.opacity(0.08), in: RoundedRectangle(cornerRadius: 14))
                    Text("学习闭环：学方法 → 做任务 → 写作品 → 看错因 → 再迁移").font(.caption).foregroundStyle(.pink).multilineTextAlignment(.center).padding(.vertical, 8)
                    Text("ChinTop · 语文登顶  v1.0.0\n本地保存学习轨迹，数据只保留在本设备。").font(.caption).multilineTextAlignment(.center).foregroundStyle(.white.opacity(0.45)).padding(.vertical, 18)
                    Button(action: onEnter) { Label("开启语文登顶之旅", systemImage: "arrow.right").font(.headline).frame(maxWidth: .infinity).padding().background(.pink, in: RoundedRectangle(cornerRadius: 14)).foregroundStyle(.white) }.padding(.bottom, 24)
